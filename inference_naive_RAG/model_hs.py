...
class TextRagInferenceModel:
    def __init__(self, config) -> None:
        ...
        self.kv_cache = None
        self.cache_size = 0
        self.max_cache_size = 10000  # 최대 캐시 크기 설정
    ...
    def update_kv_cache(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        """KV 캐시를 업데이트합니다."""
    ...
    def get_from_kv_cache(self, input_ids: torch.Tensor) -> Optional[Tuple[torch.Tensor, torch.Tensor]]:
        """KV 캐시에서 이전 상태를 가져옵니다."""
        if self.kv_cache is None:
            return None
    ...
    def generate_with_text_augmentation(self, input_text, max_length=512, temperature=0.7, confidence_threshold=0.3):
        """
        입력 텍스트로부터 생성하는 도중 문장 단위로 확신도를 측정하고, 
        확신도가 낮은 경우에만 FAISS를 이용해 관련 정보를 검색하여 augment합니다.
        </think> 토큰이 생성된 이후에는 텍스트 RAG를 중단합니다.
        ...
        try:
            ...
            with torch.no_grad():
                    ...
                    if next_token.item() in sentence_end_tokens and vector_rag_enabled:
                        ...
                        if avg_prob_gap < confidence_threshold and len(recent_sentences) > 10:
                            ...
                            if top_result:
                                try:
                                    # 검색된 문서 내용 로깅
                                    logging.info(f"검색된 유사 문서 (similarity={top_result['similarity']:.4f}): '{top_result['document'][:200]}...'")
                                    
                                    # 문맥 증강을 위한 새 입력 구성
                                    # 검색된 문서를 텍스트로 추가
                                    augmented_text = current_text + f"\n{top_result['document']}\n"
                                    ...
                                    rag_info_entry = {
                                        ...
                                        "retrieved_doc": top_result['document'][:100],
                                    ...
                                    logging.info(f"텍스트 증강 후 생성 결과: '{augmented_result[-200:]}...'")
                                    
                                    # [CTX] 토큰 추가 횟수 증가
                                    ctx_tokens_added += 1
                                    
                                    # 로그에 기록
                                    logging.info(f"텍스트 증강 성공! 총 [CTX] 토큰 추가 횟수: {ctx_tokens_added}")
                                    
                                    # 이전 문장 내역을 초기화하여 새로운 컨텍스트에서 시작
                                    recent_sentences = []
                                    
                                except Exception as e:
                                    logging.error(f"텍스트 증강 중 오류 발생: {e}", exc_info=True)
                ...
                # RAG 수행 정보 요약
                logging.info(f"생성 완료! 총 {ctx_tokens_added}번의 텍스트 증강 수행")
            ...
            logging.error(f"generate_with_text_augmentation 함수 오류: {e}", exc_info=True)
            # 오류 발생 시 입력 텍스트 반환
