        ...
        try:
            ...
            with torch.no_grad():
                ...
                while gen_len < max_new_tokens:
                    ...
                    last_token_embed = self.model.get_input_embeddings()(last_token_id)
                    
                    # KV 캐시를 활용한 다음 토큰 예측 (임베딩 사용)
                    outputs = self.model(
                    ...
                    # EOS 토큰이 생성되면 중단 (순서 변경: 토큰 추가 직후 바로 확인)
                    ...
                    if next_token.item() in sentence_end_tokens and vector_rag_enabled:
                        ...
                        if avg_prob_gap < confidence_threshold and len(recent_sentences) > 10:
                            ...
                            recent_sentences = recent_sentences[-1:]
                            ...
                            if top_results:
                                try:
                                    ...
                                    # 생성 토큰 공간에 [CTX] 토큰 추가 (실제 텍스트에는 나타나지 않음)
                                    ctx_token_tensor = torch.tensor([[ctx_token_id]], device=self.device)
                                    generated_tokens = torch.cat([generated_tokens, ctx_token_tensor], dim=-1)
                                    
                                    # [CTX] 토큰으로 증강된 생성 토큰 기록
                                    logging.info(f"생성 토큰에 [CTX] 토큰 추가됨, 현재 길이: {generated_tokens.size(1)}")
                                    ...
                                    # [CTX] 토큰의 임베딩을 프로젝터를 통과한 벡터로 대체
                                    # 여기서는 마지막 토큰으로 [CTX]를 직접 임베딩으로 사용
                                    ctx_embedding = projected_embedding.clone()
                                    
                                    # 모델이 bfloat16으로 로드되었으므로 임베딩도 bfloat16으로 변환
                                    ctx_embedding = ctx_embedding.to(torch.bfloat16)
                                    
                                    # 모델 입력 형태로 임베딩 재구성 [batch_size, sequence_length, hidden_size]
                                    # projected_embedding의 형태 로깅
                                    logging.info(f"프로젝터 출력 임베딩 형태: {ctx_embedding.shape}")
                                    
                                    # LLaMA/Mistral 모델이 기대하는 형태로 임베딩 조정
                                    # 현재 hidden_dim 확인
                                    hidden_dim = self.model.config.hidden_size
                                    logging.info(f"모델 hidden_size: {hidden_dim}")
                                    
                                    # 임베딩 재구성: [1, 1, hidden_size]
                                    if ctx_embedding.dim() == 2:
                                        ctx_embedding = ctx_embedding.reshape(1, 1, hidden_dim)
                                    elif ctx_embedding.dim() == 3 and ctx_embedding.size(1) != 1:
                                        ctx_embedding = ctx_embedding[:, 0, :].unsqueeze(1)  # 첫 번째 토큰만 선택하여 사용
                                    
                                    # 재구성된 임베딩 형태 로깅
                                    logging.info(f"재구성된 임베딩 형태: {ctx_embedding.shape}, 데이터 타입: {ctx_embedding.dtype}")
                                    
                                    # 마지막 토큰의 임베딩을 변환된 임베딩으로 교체
                                    # KV 캐시를 활용한 다음 토큰 예측 (임베딩 사용)
                                    outputs = self.model(
                                        inputs_embeds=ctx_embedding,
                                        past_key_values=past_key_values,
                                        use_cache=True
                                    )
                                    ...
                                    # EOS 토큰 확인 (RAG 이후에도 체크)
                                    if next_token.item() == self.tokenizer.eos_token_id:
                                        logging.info("RAG 후 EOS 토큰 생성, 생성 중단")
                                        break
                                    ...
                                    # 생성 결과 확인
                                    augmented_result = self.tokenizer.decode(generated_tokens[0], skip_special_tokens=True)
